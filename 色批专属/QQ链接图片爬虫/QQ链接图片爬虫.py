...
try:
    for url in url_list:
        ...
        #  随机调用User-Agent
        headers_1 = {'User-Agent': random.choice(headers_list_1)}
        response_html = requests.get(url=url, headers=headers_1, verify=False)
        #  网页转码
        response_html.encoding = 'UTF-8'
        data_html = etree.HTML(response_html.text)
        #  通过xpath规则获取访问密码文本
        passwd_text = data_html.xpath("//meta[starts-with(@name,'description')]/@content")[0]
        #  将获取的密码文本进行处理
        passwd = passwd_text.split('：')[-1]
        ...
        #  随机调用User-Agent
        headers_2 = {'User-Agent': random.choice(headers_list_2),
                   'Cookie': 'pwd=' + str(passwd)
                   }
        response_html = requests.get(url=url,headers=headers_2,verify=False)
        #  网页转码
        response_html.encoding = 'UTF-8'
        data_html = etree.HTML(response_html.text)
        #  使用xpath提取图片链接与文章名
        ...
        for pic_url in pic_url_list:
            ...
            #  发送图片下载请求
            img = requests.get(pic_url,headers=headers_2,verify=False)
            #  打开文件夹
